...
(function () {
    ...
    const tab$1 = editor => {
      const availablePlugins = () => {
        const Plugins = filter(urls, ({type}) => {
          return type === '';
        });
        const sortedPlugins = sort(map(Plugins, p => p.name), (s1, s2) => s1.localeCompare(s2));
        const PluginList = map(sortedPlugins, pluginName => `<li>${ pluginName }</li>`).join('');
        return '<div>' + '<p><b>' + global$2.translate(' plugins:') + '</b></p>' + '<ul>' + PluginList + '<li class="tox-help__more-link" ">' + '<a href="https://www.tiny.cloud/pricing/?utm_campaign=help_dialog_plugin_tab&utm_source=tiny&utm_medium=referral&utm_term=read_more&utm_content=_plugin_heading" rel="noopener" target="_blank"' + ' data-alloy-tabstop="true" tabindex="-1">' + global$2.translate('Learn more...') + '</a></li>' + '</ul>' + '</div>';
      };
      ...
      const getPluginData = (editor, key) => find(urls, x => {
        ...
        const name = x.type === '' ? `${ x.name }*` : x.name;
        const html = makeLink({
          name,
